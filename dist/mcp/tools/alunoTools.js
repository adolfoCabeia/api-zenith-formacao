            ...
            try {
                ...
                if (input.status) {
                    ...
                    if (statusUpper === 'PENDENTE')
                        statusEnum = StatusAluno.PENDENTE;
                    else if (statusUpper === 'REJEITADO')
                        statusEnum = StatusAluno.REJEITADO;
                    else if (statusUpper === 'APROVADO')
                        statusEnum = StatusAluno.APROVADO;
                }
                const data = {
    ...
    },
    {
        ...
        handler: async (input) => {
            try {
                ...
                if (input.telefone?.trim())
                    updateData.telefone = input.telefone.trim();
                if (input.turmaId?.trim())
                    updateData.turmaId = input.turmaId.trim();
                if (input.status) {
                    const statusUpper = input.status.toUpperCase();
                    if (statusUpper === 'PENDENTE')
                        updateData.status = StatusAluno.PENDENTE;
                    else if (statusUpper === 'REJEITADO')
                        updateData.status = StatusAluno.REJEITADO;
                    else if (statusUpper === 'APROVADO')
                        updateData.status = StatusAluno.APROVADO;
                }
                const files = {};
                if (input.biUrl && input.biUrl.trim() !== '') {
                    try {
