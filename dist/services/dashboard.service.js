...
export const dashboardService = {
    ...
    calc: async () => {
        ...
        const taxaAprovacao = totalAlunos > 0
            ? (alunosAprovados / totalAlunos) * 100
            : 0;
        const taxaOcupacao = turmas.map((turma) => {
            const ocupacao = (turma.alunos.length / turma.capacidade) * 100;
            return {
                turmaId: turma.id,
        ...
        const cursosRanking = cursos.map((curso) => {
            const totalAlunos = curso.turmas.reduce((acc, turma) => acc + turma.alunos.length, 0);
            return {
                curso: curso.nome,
                totalAlunos,
        ...
        cursosRanking.sort((a, b) => b.totalAlunos - a.totalAlunos);
